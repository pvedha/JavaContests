...
public class E14CollatzSequenceThisBeDamned {
	...
	private static Integer processCollatzSequence(Integer number) {
		...
		while (!largestSequenceMap.containsKey(number)) {
			...
			if (steps > largestSequence) {
				...
				// System.out.println("Breaking values : " + number + " " +
				// steps);
		...
		if(input > maxKey && largestSequence > maxSequence){
			Integer value = tmplargestSequenceMap.get(maxKey);
			tmplargestSequenceMap.remove(maxKey);
			tmplargestSequenceMap.put(input, value);
		}
		
		if(input>maxKey){
			maxKey = input;
		}
		if(maxSequence > largestSequence){
